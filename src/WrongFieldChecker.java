...
public class WrongFieldChecker implements Checker {
    ...
    private void checkWithoutAsking(Movie movie) {
        ...
        if (movie.getName() == "") {
            movie.setName("Movie" + movie.getId());
            if (!chandged) {
                System.out.println("Name-field is wrong, it will be replaced with default value");
                chandged = true;
            }
        }
        if (movie.getOscarcount() < 0) {
            movie.setOscarsCount((long) (Math.random() * 10));
            if (!chandged) {
                System.out.println("OscarCount-field is wrong, it will be replaced with default value");
                chandged = true;
            }
        }
        if (movie.getCoordinates().getX() <= -210) {
            ...
            if (!chandged) {
                System.out.println("Coordinate-field is wrong, it will be replaced with default value");
                chandged = true;
            }
        }
    }
